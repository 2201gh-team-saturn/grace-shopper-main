...
async function seed() {
  ...
  try {
    ...
    const ladybug = await User.create({
    ...
    const successfulBW = await User.create({
    ...
    const moonRoom = await Room.create({
      name: 'Moon Room',
      description:
        'Enjoy a night sleeping on the softest clouds enjoying a 360 degree view of the moon in all her glory. Add a whole new meaning to the phrase "Goodnight Moon" in our one of a kind suite.',
      price: 130.0,
      imageUrl:
        'https://images.unsplash.com/photo-1518352724948-729151797553?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8NTR8fG1vb24lMjByb29tfGVufDB8fDB8fA%3D%3D&auto=format&fit=crop&w=700&q=60',
      available: true,
      category: 'space',
    });
    const dasBergzimmer = await Room.create({
      ...
      price: 200.0,
      imageUrl:
        'https://images.unsplash.com/photo-1596394516093-501ba68a0ba6?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8NHx8aG90ZWwlMjByb29tfGVufDB8fDB8fA%3D%3D&auto=format&fit=crop&w=700&q=60',
      available: true,
      category: 'nature',
    });
    const queensCabin = await Room.create({
      ...
      price: 150.0,
      imageUrl:
        'https://images.unsplash.com/photo-1607712617949-8c993d290809?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8NzJ8fGhvdGVsJTIwcm9vbXxlbnwwfHwwfHw%3D&auto=format&fit=crop&w=700&q=60',
      available: true,
      category: 'rustic',
    });
    const caveSuite = await Room.create({
      ...
      price: 98.5,
      imageUrl:
        'https://www.masseriatorrecoccaro.com/immagini/sistema/news/foto/foto_267_7.jpg',
      available: true,
      category: 'nature',
    });
    const stanleySuite = await Room.create({
      ...
      price: 23.7,
      imageUrl:
        'https://images.weserv.nl/?url=https://the-take.com/images/uploads/screenprism/_constrain-1080w/Green-Bathroom.jpg&output=jpg',
      available: true,
      category: 'haunted',
    ...
    const zenRoom = await Room.create({
      name: 'Zen Room',
      description:
        'The Zen room provides guests with the most relaxing experience. Guests will have a chance to separate themselves from disruption and be completely at peace.',
      price: 300,
      imageUrl:
        'https://media.istockphoto.com/photos/luxurious-bedroom-interior-with-messy-bed-and-armchair-in-holiday-or-picture-id1326393613?b=1&k=20&m=1326393613&s=170667a&w=0&h=gEvTszklSpD_l0LmkGHoehehXLcOJs1q-Sar5lwJhE0=',
      available: true,
      category: 'nature',
    });
    const fantasyRoom = await Room.create({
      name: 'Fantasy Room',
      description:
        "Ready to be feel like you're in wonderland? Fantasy room will let all your dreams come true.",
      price: 250,
      imageUrl:
        'https://media.istockphoto.com/photos/the-clock-tower-viewpoint-picture-id1135502050?b=1&k=20&m=1135502050&s=170667a&w=0&h=7RHt0TMvSPIHErTncxstO1dhL9IypWRvmWK0i7xy5E0=',
      available: true,
      category: 'fantasy',
    });
    const spaceRoom = await Room.create({
      name: 'Space Room',
      description:
        'Ever wondered what it feels like to be an astronaut? Space room can provide you that experience!',
      price: 300,
      imageUrl:
        'https://flh.ca/media/73137/flh_20180612-luxuryspace-1.jpg?anchor=center&mode=crop&width=1200&height=600&rnd=132830010110000000',
      available: true,
      category: 'space',
    });
    const dragonRoom = await Room.create({
      name: 'Dragon Room',
      description:
        'Welcome to our most popular room, the Dragon Room. Guests will have access to ride our friendly dragon after purchasing the experience on our site.',
      price: 550,
      imageUrl:
        'https://images2.minutemediacdn.com/image/fetch/w_2000,h_2000,c_fit/https%3A%2F%2Fwinteriscoming.net%2Ffiles%2F2019%2F05%2F804-Drogon-and-Rhaegal-fly-at-Dragonstone.jpg',
      available: true,
      category: 'fantasy',
    });
    const poolRoom = await Room.create({
      name: 'Pool Room',
      description:
        'If the water is calling you, this is the room for you, have access to it at any point in the day',
      price: 350.0,
      imageUrl:
        'https://www.digsdigs.com/photos/10-the-most-cool-bedrooms-ever-554x346.jpg',
      available: false,
      category: 'water',
    });
    const shedroom = await Room.create({
      ...
      price: 350.0,
      imageUrl:
        'https://www.digsdigs.com/photos/10-the-most-cool-bedrooms-ever1-554x373.jpg',
      available: true,
      category: 'nature',
    });
    const artistLense = await Room.create({
      ...
      price: 350.0,
      imageUrl:
        'https://www.digsdigs.com/photos/10-the-most-cool-and-wacky-bedrooms-ever1-554x369.jpg',
      available: false,
      category: 'art',
    });
    ...
    const moonWalk = await Experience.create({
      ...
      imageUrl:
        'https://images.unsplash.com/photo-1446941303752-a64bb1048d54?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8MXx8bW9vbiUyMHdhbGt8ZW58MHx8MHx8&auto=format&fit=crop&w=700&q=60',
    });
    const alpenWandern = await Experience.create({
      ...
      imageUrl:
        'https://images.unsplash.com/photo-1534764945014-02d68e5680d8?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8MTd8fGFscCUyMGhpa2V8ZW58MHx8MHx8&auto=format&fit=crop&w=700&q=60',
    });
    const teaWQueen = await Experience.create({
      name: 'Tea With The Queen',
      description: `Find yourself atop Alpen ridges with cascading views of the multitude of lakes and green fields for kilometers. If you are lucky or bring snacks with you there is a good chance you can pet a Swiss cow on your outing as well!`,
      imageUrl:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ26pQkNLsjb0PMGG5eUFNIacGdKb50Ck0ZLEzeOFhVRRy7ZoabDoZ1waP94gsNzJDTJXE&usqp=CAU',
    });
    const spelunking = await Experience.create({
      ...
      imageUrl:
        'https://images.unsplash.com/photo-1600201319330-e99245e614c5?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80',
    });
    const ouijaBoard = await Experience.create({
      name: 'Ouija Board',
      description:
        "Do you wish to commune with the spirits that haunt this room? Now's your chance to talk to the evil forces behind the many mysterious caretaker deaths that have occured here.",
      imageUrl:
        'https://images.unsplash.com/photo-1571235398671-05a08a7a7e20?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1742&q=80',
    });
    const skyBoatRide = await Experience.create({
      name: 'Boat Ride through the sky',
      description:
        'Take a ride at night through the sky on a clear night and see the world through a beautiful view. Come relax, have a romantic night or a self care night that clears your mind.  ',
      imageUrl:
        'https://images.unsplash.com/photo-1534447677768-be436bb09401?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8OTN8fGZhbnRhc3klMjByb29tfGVufDB8fDB8fA%3D%3D&auto=format&fit=crop&w=800&q=60',
    });
    const narnia = await Experience.create({
      name: 'Narnia Day Trip',
      description: 'Take a day trip to Narnia',
      imageUrl:
        'https://t4.ftcdn.net/jpg/02/02/82/19/240_F_202821927_aB8PyDK4RnTx3bfYyoy9wYP40nx5vvMM.jpg',
    });
    const margicMirror = await Experience.create({
      name: 'Magic Mirror',
      description:
        "If you know Harry Potter then you know this mirror from Sorcerer's Stone. Come experience the very mirror harry sat in front of.",
      imageUrl:
    ...
    const review1 = await Review.create({
      review_text: `I cannot recomend the The Reverie Resort enough. For our birthday my twin and I came here and while they rode a dragon I was having tea with the queen and her corgies, no one had to compromisee! The range of experience the resort has to offer can truly make everyones dream come true.`,
    });
    const review2 = await Review.create({
      review_text:
        "I had a wonderful time in the Cave Suite. Our spelunking experience was phenomenal as well! It was fun to play hide an seek with my husband in the cave system. He's been the hide and seek champion for 3 straight days now!",
    });
